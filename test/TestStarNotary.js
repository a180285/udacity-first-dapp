...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name(), "HuangWei First Token")
    assert.equal(await instance.symbol(), "HWFT")
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starName = "awesome star 1";

    await instance.createStar(starName, starId1, {from: user1});
    let starNameGot = await instance.lookUptokenIdToStarInfo(starId1);
    assert.equal(starNameGot, starName);
});